...
system_train_state = state_matrix(:,1:0.8*systemSequence/connect); % A ∈ 
system_test_state = state_matrix(:,0.8*systemSequence/connect +1:systemSequence/connect); % C ∈

% Target output matrix
target_matrix = repmat(outputSequence,1,systemSequence/connect); % concatenation of outputSequence: Bl
target_train_state = target_matrix(:,1:0.8*systemSequence/connect);
target_test_state = target_matrix(:,1:0.2*systemSequence/connect);
